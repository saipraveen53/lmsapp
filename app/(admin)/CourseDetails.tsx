...
export default function CourseDetails() {
  ...
  // UPDATED: Use 1024 as breakpoint for the side-by-side layout to ensure enough space
  ...
  // --- RENDER HELPERS (To avoid code duplication) ---
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 60 }}>
        ...
        <View className={`px-4 -mt-10 ${isDesktop ? "max-w-7xl mx-auto w-full" : ""}`}>
            ...
            <View className={isDesktop ? "flex-row gap-6 items-start" : "flex-col"}>
                ...
                {/* This stays on the left (or top on mobile) */}
                <View className={isDesktop ? "w-[320px]" : "w-full mb-6"}>
                ...
                {isDesktop ? (
                    ...
                    <View className="flex-1 flex-row gap-6 items-start">
                        ...
                        {/* 1. Description Section (Left Side of Content) */}
                        <View className="flex-[1.5] bg-white rounded-2xl shadow-sm p-6">
                            {renderDescriptionContent()}
                        </View>

                        {/* 2. Curriculum Section (Right Side of Content) - SCROLLABLE */}
                        <View className="flex-1 bg-white rounded-2xl shadow-sm p-6 h-[600px]">
                            <View className="flex-row justify-between items-end mb-4 pb-2 border-b border-slate-100">
                                <Text className="text-xl font-bold text-slate-800">Curriculum</Text>
                                <Text className="text-xs text-slate-400">{course.sections?.length || 0} Sec • {course.totalLectures} Lec</Text>
                            </View>

                            {/* UPDATED: ScrollView for Curriculum List on Desktop */}
                            <ScrollView nestedScrollEnabled={true} showsVerticalScrollIndicator={true} className="flex-1">
                    ...
                    <View className="flex-1 bg-white rounded-2xl shadow-sm p-6 min-h-[500px]">
                        ...
                        {/* Curriculum (Non-scrollable container, flows with page) */}
                        <View>
