...
export default function CourseForm() {
  ...
  const fetchBunnyVideos = async () => {
    console.log("Fetch Bunny Videos button pressed");
    if (!form.libraryId) {
    ...
    try {
      ...
      if (response.status === 200 && response.data.items) {
        ...
        Alert.alert(
          "Success",
          `Loaded ${response.data.items.length} videos from library.`,
        );
      }
    } catch (error: any) {
  ...
  const openVideoSelection = (sIndex: number, lIndex: number) => {
    if (bunnyVideos.length === 0) {
      Alert.alert(
        "No Videos",
        "Please load videos using the Library ID first.",
      );
      return;
    }
        ...
        >
          ...
          <View className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 mb-6">
            ...
            <InputField
              label="Course Title *"
              value={form.title}
              onChange={(t: string) => updateField("title", t)}
              error={errors.title}
            />
            <InputField
              label="Subtitle"
              value={form.subtitle}
              onChange={(t: string) => updateField("subtitle", t)}
            />
            <InputField
              label="Description *"
              value={form.description}
              onChange={(t: string) => updateField("description", t)}
              multiline
            />
            ...
            <View className={isDesktop ? "flex-row gap-4" : "flex-col"}>
              ...
              <View className="flex-1">
                <InputField
                  label="Language"
                  value={form.language}
                  onChange={(t: string) => updateField("language", t)}
                />
              </View>
            </View>
            ...
            <View className="flex-row gap-2 mb-4">
              {["BEGINNER", "INTERMEDIATE", "ADVANCED"].map((lvl) => (
                ...
                >
                  <Text
                    className={`text-xs font-bold ${form.level === lvl ? "text-white" : "text-slate-500"}`}
                  >
                    {lvl}
                  </Text>
                </Pressable>
          ...
          <View className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 mb-6">
            ...
            <InputField
              ...
              onChange={(t: string) => updateField("whatYouWillLearn", t)}
              multiline
            />
            <InputField
              label="Requirements"
              value={form.requirements}
              onChange={(t: string) => updateField("requirements", t)}
            />
            <InputField
              label="Target Audience"
              value={form.targetAudience}
              onChange={(t: string) => updateField("targetAudience", t)}
            />

            <View className="mt-4 gap-4">
              <View className="flex-row items-center justify-between bg-slate-50 p-3 rounded-xl">
                <Text className="font-bold text-slate-600 ml-2">
          ...
          <View className="mb-6">
            <View className="flex-row items-center justify-between mb-4">
              ...
              <Pressable
                onPress={addSection}
                className="flex-row items-center bg-indigo-600 px-4 py-2 rounded-xl shadow-md shadow-indigo-200"
              >
              ...
              >
                ...
                <View className="bg-slate-50 p-4 border-b border-slate-100 flex-row justify-between items-start">
                  <View className="flex-1 mr-4">
                    ...
                    <TextInput
                      ...
                      onChangeText={(t) =>
                        updateSection(sIndex, "description", t)
                      }
                ...
                <View className="p-4 bg-slate-50/30">
                  {section.lectures.map((lecture, lIndex) => (
                    <View
                      key={lIndex}
                      className="flex-row items-center mb-3 bg-white p-3 rounded-xl border border-slate-100 shadow-sm"
                    >
                      ...
                      <View className="flex-1 mr-2 gap-2">
                        <TextInput
                          ...
                          value={lecture.title}
                          onChangeText={(t) =>
                            updateLecture(sIndex, lIndex, "title", t)
                          }
                        ...
                        <View className="flex-row items-center gap-2">
                          <TextInput
                            ...
                            onChangeText={(t) =>
                              updateLecture(sIndex, lIndex, "videoUrl", t)
                            }
                        ...
                        {lecture.duration > 0 && (
                          <Text className="text-[10px] text-slate-400">
                            Duration: {Math.floor(lecture.duration / 60)}m{" "}
                            {lecture.duration % 60}s
                          </Text>
                        )}
                      </View>
                      <Pressable
                        onPress={() => removeLecture(sIndex, lIndex)}
                        className="p-2"
                      >
                        <Ionicons
          ...
          {submitMessage && (
            <View style={{ marginTop: 8, alignItems: "center" }}>
              <Text
                style={{
                  color:
                    submitMessageType === "success" ? "#16a34a" : "#dc2626",
                  fontWeight: "bold",
                  fontSize: 16,
                }}
              >
              ...
              >
                ...
                <View className="flex-1">
                  ...
                  <Text className="text-xs text-slate-500 mt-1">
                    Duration: {Math.floor(item.length / 60)}m {item.length % 60}
                    s
                  </Text>
                </View>
